...
contract('AdapterRegistry', () => {
  ...
  beforeEach(async () => {
    ...
    await adapterRegistry.methods.addProtocols(
      ...
      [[
        ...
        '0',
        [
          [protocolAdapterAddress, [protocolAdapterAddress]],
        ],
      ]],
    )
      .send({
  ...
  it('should be correct protocol adapters', async () => {
    await adapterRegistry.methods.getProtocolAdapters('Mock')
      ...
      .then((result) => {
        assert.equal(result.length, 1);
        assert.equal(result[0].adapterAddress, protocolAdapterAddress);
        assert.equal(result[0].supportedTokens.length, 1);
        assert.equal(result[0].supportedTokens[0], protocolAdapterAddress);
      });
  });

  it('should be correct protocol info', async () => {
    await adapterRegistry.methods.getProtocolInfo('Mock')
      .call()
      .then((result) => {
  ...
  it('should not update protocol info not by the owner', async () => {
    await expectRevert(adapterRegistry.methods.updateProtocolInfo(
      'Mock',
  ...
  it('should update protocol info by the owner', async () => {
    ...
    await adapterRegistry.methods.updateProtocolInfo(
      'Mock',
      'New Mock Protocol Name',
      'New mock description',
      'New mock website',
  ...
  it('should add protocol adapter by the owner', async () => {
    ...
    await adapterRegistry.methods.getProtocolAdapters('Mock')
      ...
      .then((result) => {
        assert.equal(result.length, 2);
        assert.deepEqual(result[0], [protocolAdapterAddress, [protocolAdapterAddress]]);
        assert.deepEqual(result[1], [protocolAdapterAddress, [protocolAdapterAddress]]);
      });
  });
  ...
  it('should remove protocol adapter by the owner', async () => {
    ...
    await adapterRegistry.methods.getProtocolAdapters('Mock')
      ...
      .then((result) => {
        assert.equal(result.length, 2);
        assert.deepEqual(result[0], [ONE, [ONE]]);
        assert.deepEqual(result[1], [TWO, [TWO]]);
      });
    await adapterRegistry.methods.removeProtocolAdapters(
      'Mock',
      [
    ...
    await adapterRegistry.methods.getProtocolAdapters('Mock')
      ...
      .then((result) => {
        assert.equal(result.length, 1);
        assert.deepEqual(result[0], [TWO, [TWO]]);
      });
    await adapterRegistry.methods.addProtocolAdapters(
      'Mock',
      [
    ...
    await adapterRegistry.methods.getProtocolAdapters('Mock')
      ...
      .then((result) => {
        assert.equal(result.length, 2);
        assert.deepEqual(result[0], [TWO, [TWO]]);
        assert.deepEqual(result[1], [ONE, [ONE]]);
      });
    await adapterRegistry.methods.removeProtocolAdapters(
      'Mock',
      [
  ...
  it('should not update protocol adapter not by the owner', async () => {
    await expectRevert(adapterRegistry.methods.updateProtocolAdapterInfo(
      'Mock',
      0,
      ONE,
      [],
  ...
  it('should update protocol adapter by the owner', async () => {
    await adapterRegistry.methods.updateProtocolAdapterInfo(
      'Mock',
      0,
      ONE,
      [],
    ...
    await adapterRegistry.methods.getProtocolAdapters('Mock')
      ...
      .then((result) => {
        assert.equal(result.length, 1);
        assert.deepEqual(result[0], [ONE, [protocolAdapterAddress]]);
      });
    await adapterRegistry.methods.updateProtocolAdapterInfo(
      'Mock',
      0,
    ...
    await adapterRegistry.methods.getProtocolAdapters('Mock')
      ...
      .then((result) => {
        assert.equal(result.length, 1);
        assert.deepEqual(result[0], [ONE, [ONE, ONE]]);
      });
  });
  ...
  it('should be correct balances non-null', async () => {
    await adapterRegistry.methods.getBalances(accounts[0])
      .call()
      .then((result) => {
        assert.equal(result[0].name, 'Mock Protocol Name');
        assert.equal(result[0].description, 'Mock protocol description');
        assert.equal(result[0].websiteURL, 'Mock website');
        assert.equal(result[0].iconURL, 'Mock icon');
        assert.equal(result[0].version, '1');
        assert.equal(result[0].adapterBalances[0].adapterType, 'Asset');
        assert.deepEqual(result[0].adapterBalances[0].balances[0].base.info, mockAsset);
        assert.equal(result[0].adapterBalances[0].balances[0].base.value, 1000);
  ...
  it('should be correct balances null', async () => {
    await adapterRegistry.methods.getBalances(accounts[1])
      ...
      .then((result) => {
        ...
        assert.equal(result[0].adapterBalances[0].adapterType, 'Asset');
        assert.deepEqual(result[0].adapterBalances[0].balances[0].base.info, mockAsset);
        assert.equal(result[0].adapterBalances[0].balances[0].base.value, 0);
        assert.deepEqual(result[0].adapterBalances[0].balances[0].underlying, []);
      });
  });
});
